...
import { displayStr, getUxSettings, repr } from "../../../../../../utils";
...
export function SimpleCodeBlock(props) {
  return (<pre className="simple-code-block hl-left pl-2">
            <code
              dangerouslySetInnerHTML={{ __html: renderPythonCode(props.children) }} />
        </pre>);
}
...
class ActiveBoxSelectionUnthrottled extends React.PureComponent {
  render() {
    ...
    const style = {
      ...
      width: boxSize,
      height: boxSize,
      borderRadius, // TODO: the part after : is weird/wrong
      transform: idx != null ? computeBoxTransformProperty(idx, yOffset, boxSize, spacingX) : undefined
    };
    return <div ref={this.props.setInnerRef} className={classNames(classes)}
                style={style} />;
  }
}
...
class ActiveBoxSelectionThrottledHelper extends React.Component {
  ...
  render() {
    ...
    return (<ActiveBoxSelectionUnthrottled
...
class SelectionGroup extends React.Component {
  ...
  constructor() {
    super();
    this.state = {
      transitionRunning: false, transitionStarting: false, epoch: 0
    };
  }
...
class Box extends React.PureComponent {
  shortDisplayedString(value) {
    const extraType = value === "DUMMY" || value === "EMPTY" || value === "" || (typeof value === "string" && /^[-+]?[0-9]+$/.test(value));
    let isEmpty = false;
    ...
    if (s.length <= maxLen) {
      ...
      if (cutCharsCount === 4) {
        shortenedValue = [s.substring(0, cutCharsCount),
          <br key="br1" />, "\u22EF",
          <br key="br2" />, s.substring(s.length - cutCharsCount, s.length)];
      } else {
        shortenedValue = [s.substring(0, cutCharsCount), "\u2026", s.substring(s.length - cutCharsCount, s.length)];
      }
    }
    return {
      shortenedValue, extraType, isEmpty
    };
  }
  ...
  render() {
    ...
    if (value != null) {
      ...
      if (shortenedValue.length === 1 && shortenedValue[0] === "" /* TODO FIXME: this check is kidna ugly & is a leaky abstraction */) {
        extraTypeSpan = (<span className="box-content-extra-type" style={style}>
                        (empty str)
                    </span>);
      } else {
      ...
      content = (<span className="box-content">
    ...
    return (<>
      <div
        style={{
          transform: status !== "removed" ? computeBoxTransformProperty(idx, y, boxSize, spacingX, 0) : "translate(0px, 0px)",
          width: boxSize,
          height: boxSize,
          fontSize: fontSize,
          lineHeight: `${fontSize}px`,
          borderRadius,
          padding: boxPadding, ...extraStyle
        }}
        className={classNames(classes)}
      />
      {content && (<div
        style={{
          transform: status !== "removed" ? computeBoxTransformProperty(idx, y, boxSize, spacingX, 5) : "translate(0px, 0px)",
          width: boxSize,
          height: boxSize,
          fontSize: fontSize,
          lineHeight: `${fontSize}px`,
...
class SlotSelection extends React.PureComponent {
  render() {
    ...
    const individualSelectionsProps = [{
      key: `${extraClassName}-hashCode`, extraClassName, color, yOffset: 0
    }, {
      key: `${extraClassName}-key`,
      extraClassName,
      color,
      yOffset: boxSize + spacingY
    }, {
...
class BaseBoxesComponent extends React.PureComponent {
  ...
  static markRemoved(state, targetModId) {
    ...
    if (updatedCount > 0) {
      ...
      return {
        keyData: state.keyData.mergeDeep(toMerge), needReflow: true, gcModId
      };
    } else {
      return null;
    }
  ...
  static getDerivedStateFromProps(nextProps, state) {
    ...
    if (geometryChanged) {
      ...
      state = {
        ...state, keyData: state.keyData.mergeDeep(toMerge), boxGeometry
      };
    }
    ...
    if (!state.firstRender) {
      ...
      if (nextArrayPreConversion !== state.lastNextArrayPreConversion) {
        ...
        const newBox = (key, idx, someProps, group, value) => {
          ...
          const box = <Box idx={idx} status="created"
                           key={id} {...actualGeometry} {...someProps} />;
        ...
        for (let idx = 0; idx < nextArray.length; ++idx) {
          ...
          for (const [group, [key, someProps]] of idxBoxesProps.entries()) {
            ...
            if (!oldData) {
              ...
              if (status !== "adding" || oldData.idx !== idx || oldData.someProps.yOffset !== someProps.yOffset || oldData.someProps.yRel !== someProps.yRel) {
                ...
                const newBox = React.cloneElement(box, {
                  idx, status: newStatus, ...someProps
                });
                toMerge[key] = {
                  box: newBox,
                  status: newStatus,
                ...
                BaseBoxesComponent.notSoDeepDel(newRemovingValueToGroupToKeyToId, [repr(value, true), group, key]);
              }
            }
          }
        }
        ...
        const recycleId = (key, idx, value, groupOfKeyToId, keyToId) => {
          ...
          BaseBoxesComponent.notSoDeepDel(newRemovingValueToGroupToKeyToId, [repr(value, true), groupOfKeyToId, keyWithRecycledId]);
        ...
        for (let key in notExistingKeyToData) {
          ...
          if (key in keyToRecycledBox) {
            ...
            const newBox = React.cloneElement(box, {
              idx: idx, status: newStatus, ...data.someProps
            });
            toMerge[key] = new BBRecord({
              box: newBox,
              idx,
      ...
      for (let idx = 0; idx < nextArray.length; ++idx) {
        ...
        for (const [group, [key, someProps]] of idxBoxesProps.entries()) {
          const value = someProps.value;
          const id = (++lastBoxId).toString();
          const box = <Box idx={idx} key={id}
                           status="adding" {...boxGeometry} {...someProps} />;
    ...
    const getOrModSelection = (selection, extraClassName, oldIdx, _idx, status, color) => {
      ...
      if (!selection) {
        ...
        return [React.cloneElement(selection, {
          idx, status, ...boxGeometry
        }), status];
      }
    };

    if (activeBoxSelection1 || nextProps.idx != null) {
      [activeBoxSelection1, activeBoxSelection1status] = getOrModSelection(activeBoxSelection1, "active-box-selection-1", state.lastIdx, nextProps.idx, activeBoxSelection1status, nextProps.selection1color || RED);
    }

    if (activeBoxSelection2 || nextProps.idx2 != null) {
      [activeBoxSelection2, activeBoxSelection2status] = getOrModSelection(activeBoxSelection2, "active-box-selection-2", state.lastIdx2, nextProps.idx2, activeBoxSelection2status, nextProps.selection2color || BLUE);
    }
  ...
  render() {
    ...
    if (this.state.needGarbageCollection) {
      ...
      setTimeout(() => this.updateModIdForGC(currentModificationId), BaseBoxesComponent.ANIMATION_DURATION_TIMEOUT);
    }
    ...
    return (<div className="hash-vis" style={{ height: this.props.height }}
                 ref={this.ref}>
      {boxes}
    </div>);
  }
  ...
  componentDidUpdate() {
    ...
    if (this.state.needProcessCreatedAfterRender) {
      ...
      console.log("BaseBoxesComponent.componentDidUpdate needProcessCreatedAfterRender==true timing", performance.now() - t1);
    }
  }
}
...
export function TetrisFactory(lines, opts) {
  ...
  return class extends React.PureComponent {
    ...
    render() {
      return (
        <Tetris lines={lines} {...this.props} innerRef={this.props.innerRef}
...
export class Tetris extends React.PureComponent {
  ...
  static _getExpectedHeight(windowWidth, windowHeight, lines, fixedGeometry) {
    ...
    return (hackyPaddingSize + this.VIS_MARGIN * (lines.length - 1) + _.sum(lines.map(([Component, [ld, d, i, i2, subProps]]) => Component.getExpectedGeometry({ boxGeometry, ...subProps }).height)));
  }
  ...
  render() {
    ...
    for (let [i, [Component, [linesData, dataName, idxName, idx2Name, subProps]]] of props.lines.entries()) {
      ...
      const {
        rowMarginBottom, rowHeight, height, rowsNumber
      } = Component.getExpectedGeometry({
        boxGeometry, ...subProps
      });
      labelsEnabled = labelsEnabled || !linesData.labels.every((label) => label == null);

      labels = labels.concat(linesData.labels.map((label, index) => {
        const marginBottom = index === linesData.labels.length - 1 ? tetrisRowMarginBottom : rowMarginBottom;
        const expectedNumLabels = rowsNumber;
        const actualNumLabels = linesData.labels.length;
        if (actualNumLabels != expectedNumLabels && actualNumLabels != 1) throw new Error(`Mismatched number of labels`);
        return (<div
      ...
      elems.push(<div className="tetris-row" key={`row-${i}`}
                      style={{ marginBottom: tetrisRowMarginBottom }}>
    ...
    return (<SmoothScrollbar alwaysShowTracks={true} style={style}
...
export class CodeBlockWithActiveLineAndAnnotations extends React.PureComponent {
  ...
  getCodeWithExplanationHtmlLines(visibleBreakpoints, activeBp) {
    ...
    for (let i = 0; i < code.length; ++i) {
      ...
      let formattedLine = (<pre className="code-line-container" key="pre">
                    <code>
                        <span
  ...
  render() {
    ...
    const {
      lines, highlightedExplanationContent
    } = this.getCodeWithExplanationHtmlLines(visibleBreakpoints, activeBp);
    ...
    return (<>
      ...
      >
        {this.props.mobileHeaderTitle && (<div
          className="mobile-header-title">{this.props.mobileHeaderTitle}</div>)}
        <div
          style={{
      ...
      {this.props.withShortExplanation && (<div
  ...
  getScrollTopTarget() {
    ...
    const needsUpdating = activeLinePos < scrollTop + 1.2 * estimatedLineHeight || activeLinePos > scrollBottom - 2.0 * estimatedLineHeight;
...
class TimeSliderWithControls extends React.Component {
  ...
  constructor() {
    super();
    this.state = {
      time: null, autoPlaying: false, speed: 1, userInteracted: false
    };
    this.timeoutId = null;
  ...
  setSpeed = (speed) => {
    if (speed !== globalSettings.speed) {
      if (this.state.autoPlaying && this.getAutoplayTimeout() - (this.unixtimestamp() - this.timeoutStarted) > this.getAutoplayTimeout(speed)) {
        clearTimeout(this.timeoutId);
  ...
  render() {
    ...
    for (let i = 0; i < speeds.length; ++i) {
      ...
      speedControls.push(<label key={label}
                                className={classNames("btn", "btn-outline-dark", { active: isActive })}>
        <input type="radio" checked={isActive}
               onChange={() => this.setSpeed(speed)} /> {label}
      </label>);
    }
      ...
      <div className="row">
        <div className="col-sm-12 col-md-12">
          <div className="btn-toolbar slider-controls">
            <div
              className="btn-group btn-group-sm mr-2 mb-1">{timeControls}</div>
            <div
              className="btn-group btn-group-sm btn-group-toggle mr-2 mb-1">{speedControls}</div>
          </div>
        </div>
      </div>
      <div className="row slider-row fix-animation">
        <div className="col-md-6 col-sm-12">
          <Slider
            ...
            value={time}
            dotStyle={{
              top: -1, height: 12, width: 12
            }}
            handleStyle={{
              height: 20, width: 20, marginTop: -6
            }}
            railStyle={{ height: 10 }}
            trackStyle={{
              height: 10
...
export class VisualizedCode extends React.Component {
  ...
  render() {
    ...
    if (windowHeight) {
      ...
      codeHeight = this.props.windowHeight - StateVisualization.getExpectedHeight(windowWidth, windowHeight) - approximateSliderAndControlsHeight - extraSpace;
      if (codeHeight < 225) {
        codeHeight += 50;
      }
    ...
    let time = this.props.keepTimeOnNewBreakpoints ? this.state.userAdjustedToMax ? this.props.breakpoints.length - 1 : Math.min(this.state.time, this.props.breakpoints.length - 1) : this.state.time;
      ...
      <div className="visualized-code hl-left">
        ...
        <DebounceWhenOutOfView
          childProps={{
            ...
            bp: bp, /* the breakpoints and bpIdx is for chapter4 probing visualization */
            breakpoints: this.props.breakpoints,
            bpIdx: time,
            epoch: this.state.breakpointsUpdatedCounter
          }}
          windowHeight={windowHeight}
          childFunc={(props, innerRef) => (<StateVisualization
            {...props}
            innerRef={innerRef}
        ...
        />
        {this.props.comment}
        {this.props.code && (<div className="row code-block-row fix-animation">
          <div className="col">
...
export class HashSlotsComponent extends React.PureComponent {
  ...
  static boxFactory(keys, value) {
    const slot = value;
    return [[keys[1], {
      value: slot.key, yOffset: 0
    }], [keys[2], {
      value: slot.value, yRel: 1
  ...
  static getKeys(array) {
    ...
    return array.map((slot, idx) => {
      ...
      return [emptyOrKey(slot.pyHashCode, idx, "hashCode", keyKey), emptyOrKey(slot.key, idx, "key", keyKey), emptyOrKey(slot.value, idx, "value", keyKey)];
    });
  }
